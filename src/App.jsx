        ...
        <div style={{padding:"0 24px"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:16}}>
            ...
            <a href={`https://wa.me/?text=${waText}`} target="_blank" rel="noreferrer" style={{
              background:"#25D366",color:"white",borderRadius:14,padding:"14px 20px",
              textDecoration:"none",display:"flex",alignItems:"center",gap:12,fontWeight:700,fontSize:15
            }}>
            ...
            <div style={{background:"white",border:"1px solid #E9D5FF",borderRadius:14,overflow:"hidden"}}>
              ...
              <div style={{padding:"10px 14px",display:"flex",gap:8}}>
                {[["story","📖 Story"],["bio","👤 Bio"],["dm","💬 DM"]].map(([id,label])=>(
                  <button key={id} onClick={()=>copyIg(id)} style={{
                    flex:1,background:igCopied===id?"#F3E8FF":T.surface2,
                    border:`1px solid ${igCopied===id?"#C084FC":T.border}`,
                    borderRadius:10,padding:"8px 4px",fontSize:11,
                    fontWeight:600,color:igCopied===id?"#7C3AED":T.text,cursor:"pointer"
                  }}>
            ...
            <a href={`mailto:?subject=Lista de regalos 🎁&body=¡Hola! Te comparto la lista de ${ownerName}: ${link}`} style={{
              background:T.accentL,color:T.accent,borderRadius:14,padding:"14px 20px",
              textDecoration:"none",display:"flex",alignItems:"center",gap:12,
              fontWeight:700,fontSize:15,border:`1px solid ${T.accent}30`
            }}>
            ...
            <button className="btn" onClick={()=>{navigator.clipboard.writeText(link);setCopied(true);setTimeout(()=>setCopied(false),2400);}} style={{
              ...
              padding:"14px 20px",fontSize:15,fontWeight:700,transition:"background .3s",
...
function ListDetail({ list, user, onBack, onUpdateItems, viewMode, setViewMode }) {
  ...
  const takeItem = async (id,by) => {
    ...
    await onUpdateItems(list.id, updated);
    burst();
    // Mejora 3 — notificación si lista completa
